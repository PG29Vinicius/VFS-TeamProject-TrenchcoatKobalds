...
/// <summary>
/// Detects when the player enters or exits the search range of an enemy.
/// Works with both MeleeEnemyController and RangeEnemyController.
/// </summary>
public class SearchRangeDetector : MonoBehaviour
{
    ...
    /// <summary>
    /// Detects when the player enters the search range trigger and notifies the parent enemy controller.
    /// </summary>
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            {
                _meleeEnemyController.OnPlayerEnterSearchRange();
            }
            else if (_rangeEnemyController != null)
            {
                _rangeEnemyController.OnPlayerEnterSearchRange();
            }
        }
    }

    /// <summary>
    /// Detects when the player exits the search range trigger and notifies the parent enemy controller.
    /// </summary>
    ...
    void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            if (_meleeEnemyController != null)
            {
                _meleeEnemyController.OnPlayerExitSearchRange();
            }
            else if (_rangeEnemyController != null)
            {
                _rangeEnemyController.OnPlayerExitSearchRange();
            }
        }
    }
}